...
function normalizeNowPaymentsApiKey(value) {
  const raw = String(value || '').trim()
  const keyValueLine = raw
    .split(/\r?\n/)
    .map((line) => line.trim())
    .find((line) => /^NOWPAYMENTS_(API_KEY|KEY)\s*=/.test(line))
  const selected = keyValueLine ? keyValueLine.slice(keyValueLine.indexOf('=') + 1).trim() : raw
  return selected.replace(/[^\x20-\x7e]/g, '').trim()
}
...
async function getNowPaymentsSettings(env) {
  const apiKey = normalizeNowPaymentsApiKey(await resolveSecretValue(env?.NOWPAYMENTS_API_KEY ?? env?.NOWPAYMENTS_KEY))
  return {
    apiKey,
