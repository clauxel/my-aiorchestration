...
declare global {
  interface Window {
    aiorchestrationTrack?: (name: string, metadata?: Record<string, unknown>) => void;
  }
}

const visitorStorageKey = "aiorchestration-analytics-visitor";
const sessionStorageKey = "aiorchestration-analytics-session";
const queueStorageKey = "aiorchestration-analytics-queue";
...
function getSessionId() {
  ...
  try {
    ...
    if (raw) {
      ...
      if (parsed.id && parsed.seenAt && now - parsed.seenAt < sessionInactivityMs) {
...
export default function Analytics() {
  useEffect(() => {
    loadQueue();
    window.aiorchestrationTrack = trackEvent;
    ...
    return () => {
      ...
      if (window.aiorchestrationTrack === trackEvent) delete window.aiorchestrationTrack;
    };
  }, []);
